...
if (isset($_POST['update_profile'])) {
    ...
    if ($upd->execute()) {
        ...
        $message = "<p class='error'><i class=\"fas fa-times-circle\"></i> Failed to update profile.</p>";
    }
}
...
if (isset($_POST['change_password'])) {
    ...
    if (!password_verify($current, $user['password_hash'])) {
        $message = "<p class='error'><i class=\"fas fa-times-circle\"></i> Current password is incorrect.</p>";
    } elseif ($new !== $confirm) {
        $message = "<p class='error'><i class=\"fas fa-times-circle\"></i> New passwords do not match.</p>";
    } elseif (strlen($new) < 6) {
        $message = "<p class='error'><i class=\"fas fa-times-circle\"></i> Password must be at least 6 characters.</p>";
    } else {
        $hash = password_hash($new, PASSWORD_DEFAULT);
        $upd = $conn->prepare("UPDATE users SET password_hash=? WHERE id=?");
        $upd->bind_param("si", $hash, $uid);
        if ($upd->execute()) {
            $message = "<p class='success'><i class=\"fas fa-check-circle\"></i> Password changed successfully.</p>";
        } else {
            $message = "<p class='error'><i class=\"fas fa-times-circle\"></i> Failed to change password.</p>";
        }
    }
}
?>
...
<head>
  ...
  <title>Admin Profile - IFM Peer Tutoring</title>
  ...
  <div class="container">
    ...
    <div class="info-panel">
      <div class="profile-pic-wrapper">
        <img src="../uploads/profile_pics/<?php echo !empty($user['profile_pic']) ? $user['profile_pic'] : 'default.png'; ?>" alt="Profile Picture" id="current-profile-pic">
      </div>
    ...
    <div class="section">
      <h3><span><i class="fas fa-key"></i></span> Change Password</h3>
      <form method="POST">
        <div class="form-group">
        ...
        <div class="form-group">
          <label for="confirm_password"><span>🔐</span> Confirm New Password</label>
